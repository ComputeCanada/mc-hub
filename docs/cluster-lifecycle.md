...
Destroy removes only an empty cluster definition. The backend locks the workspace, checks all pages of runs for pending work, and checks the latest Terraform state for managed resource instances. Failed checks retain the cluster and release the lock. Finish or discard pending Terraform runs before retrying. Successful destruction archives the repository, tags the workspace `deleted`, leaves that workspace locked, and removes the cluster record. Neither the repository nor the workspace is permanently deleted. Clusters that failed before workspace creation can also be removed after teardown.
